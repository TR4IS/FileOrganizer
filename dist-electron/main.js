...
const TEMP_EXTENSIONS = /* @__PURE__ */ new Set([
  ".crdownload",
  ".part",
  ".part1",
  ".part2",
...
class Organizer {
  constructor(targetPath, rules, logger, retryLimit = 720) {
    __publicField(this, "extensionMap");
    __publicField(this, "retryTracker", /* @__PURE__ */ new Map());
    __publicField(this, "retryLimit");
  ...
  organize() {
    ...
    for (const folder of folderNames) {
      const folderPath = path$p.join(this.targetPath, folder);
      if (!fs$n.existsSync(folderPath)) {
        try {
          fs$n.mkdirSync(folderPath, { recursive: true });
          createdFolders++;
    ...
    let entries;
    try {
      entries = fs$n.readdirSync(this.targetPath, { withFileTypes: true });
    } catch (err) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const dest2 = path$p.join(this.targetPath, "random", entry.name);
        if (moveItem(path$p.join(this.targetPath, entry.name), dest2, this.logger)) {
          movedDirectories++;
      ...
      const ext = path$p.extname(entry.name).toLowerCase();
      if (TEMP_EXTENSIONS.has(ext)) continue;
      const filePath = path$p.join(this.targetPath, entry.name);
      ...
      const dest = path$p.join(this.targetPath, folder, entry.name);
...
function buildExtensionMap(rules) {
  const map2 = /* @__PURE__ */ new Map();
  for (const rule of rules) {
    const ext = rule.extension.toLowerCase();
    if (!map2.has(ext)) map2.set(ext, rule.folder);
  }
  return map2;
}
function isFileReady(filePath) {
  try {
    const fd = fs$n.openSync(filePath, "r+");
    fs$n.closeSync(fd);
    return true;
  } catch {
    return false;
...
function moveItem(src2, dest, logger) {
  try {
    fs$n.renameSync(src2, dest);
    return true;
  } catch (err) {
    if (isExdevError(err)) {
      try {
        fs$n.copyFileSync(src2, dest);
        fs$n.unlinkSync(src2);
        return true;
      } catch (copyErr) {
        logger(`[!] Failed to copy ${path$p.basename(src2)}: ${copyErr}`);
        return false;
      }
    }
    logger(`[!] Failed to move ${path$p.basename(src2)}: ${err}`);
    return false;
  }
}
function isExdevError(err) {
  return typeof err === "object" && err !== null && err.code === "EXDEV";
}
...
const scan$1 = (input, options) => {
  ...
  if (opts.unescape === true) {
    if (glob) glob = utils$8.removeBackslashes(glob);
    if (base && backslashes === true) {
      base = utils$8.removeBackslashes(base);
    }
  }
  const state = {
    prefix,
...
const getStarExtglobSequenceOutput = (pattern) => {
  ...
  const source = chars2.length === 1 ? utils$7.escapeRegex(chars2[0]) : `[${chars2.map((ch) => utils$7.escapeRegex(ch)).join("")}]`;
  return `${source}*`;
};
...
const analyzeRepeatedExtglob = (body, options) => {
  ...
  const max = typeof options.maxExtglobRecursion === "number" ? options.maxExtglobRecursion : constants$6.DEFAULT_MAX_EXTGLOB_RECURSION;
...
const parse$a = (input, options) => {
  ...
  const PLATFORM_CHARS = constants$6.globChars(win32);
  ...
  const extglobClose = (token) => {
    ...
    if ((token.type === "plus" || token.type === "star") && analysis.risky) {
      ...
      open2.type = "text";
      open2.value = literal;
      open2.output = safeOutput || utils$7.escapeRegex(literal);
  ...
  while (!eos()) {
    ...
    if (value === "]") {
      ...
      const escaped2 = utils$7.escapeRegex(prev.value);
...
const picomatch$3 = (glob, options, returnState = false) => {
  ...
  const opts = options || {};
  const posix = utils$6.isWindows(options);
...
picomatch$3.test = (input, regex, options, { glob, posix } = {}) => {
  ...
  const opts = options || {};
  const format = opts.format || (posix ? utils$6.toPosixSlashes : null);
...
picomatch$3.matchBase = (input, glob, options, posix = utils$6.isWindows(options)) => {
  const regex = glob instanceof RegExp ? glob : picomatch$3.makeRe(glob, options);
  return regex.test(path$n.basename(input));
};
picomatch$3.isMatch = (str2, patterns, options) => picomatch$3(patterns, options)(str2);
...
picomatch$3.constants = constants$5;
...
const readdir$1 = promisify$3(fs$m.readdir);
const stat$8 = promisify$3(fs$m.stat);
const lstat$2 = promisify$3(fs$m.lstat);
const realpath$1 = promisify$3(fs$m.realpath);
const BANG$2 = "!";
...
class ReaddirpStream extends Readable {
  ...
  constructor(options = {}) {
    ...
    const statMethod = opts.lstat ? lstat$2 : stat$8;
    ...
    this._isDirent = "Dirent" in fs$m && !opts.alwaysStat;
...
const readdirp$1 = (root2, options = {}) => {
  let type2 = options.entryType || options.type;
  if (type2 === "both") type2 = FILE_DIR_TYPE;
  if (type2) options.type = type2;
  if (!root2) {
...
const readdirpPromise = (root2, options = {}) => {
  return new Promise((resolve, reject) => {
    const files = [];
    readdirp$1(root2, options).on("data", (entry) => files.push(entry)).on("end", () => resolve(files)).on("error", (error2) => reject(error2));
  });
};
...
var strictCheck = function(str2) {
  ...
  while (index < str2.length) {
    ...
    if (str2[index] === "\\") {
      ...
      if (close2) {
        var n = str2.indexOf(close2, index);
        if (n !== -1) {
          index = n + 1;
        }
      }
...
var isGlob$2 = function isGlob(str2, options) {
  if (typeof str2 !== "string" || str2 === "") {
    return false;
  }
  if (isExtglob2(str2)) {
...
var stringify$9 = (ast, options = {}) => {
  const stringify2 = (node2, parent = {}) => {
    const invalidBlock = options.escapeInvalid && utils$4.isInvalidBrace(parent);
    const invalidNode = node2.invalid === true && options.escapeInvalid === true;
    let output = "";
    if (node2.value) {
      if ((invalidBlock || invalidNode) && utils$4.isOpenOrClose(node2)) {
        return "\\" + node2.value;
      }
      return node2.value;
    }
...
const stringify$8 = (start, end, options) => {
...
const fillNumbers = (start, end, step = 1, options = {}) => {
  ...
  let toNumber = padded === false && stringify$8(start, end, options) === false;
...
const compile$1 = (ast, options = {}) => {
  const walk = (node2, parent = {}) => {
    const invalidBlock = utils$3.isInvalidBrace(parent);
    const invalidNode = node2.invalid === true && options.escapeInvalid === true;
    ...
    if (node2.nodes && node2.ranges > 0) {
      const args = utils$3.reduce(node2.nodes);
...
const expand$1 = (ast, options = {}) => {
  ...
  const walk = (node2, parent = {}) => {
    ...
    if (node2.invalid || node2.dollar) {
      q.push(append(q.pop(), stringify$7(node2, options)));
      return;
    }
  ...
  while (index < length) {
    ...
    if (value === CHAR_COMMA$1 && depth2 > 0) {
      if (block.ranges > 0) {
        ...
        block.nodes = [open2, { type: "text", value: stringify$6(block) }];
      }
...
braces$1.stringify = (input, options = {}) => {
  if (typeof input === "string") {
    return stringify$5(braces$1.parse(input, options), options);
  }
  return stringify$5(input, options);
};
...
var isBinaryPath$1 = (filePath) => extensions.has(path$m.extname(filePath).slice(1).toLowerCase());
var constants$3 = {};
(function(exports) {
  const { sep } = path$p;
...
const open = promisify$2(fs$l.open);
const stat$7 = promisify$2(fs$l.stat);
const lstat$1 = promisify$2(fs$l.lstat);
const close = promisify$2(fs$l.close);
const fsrealpath = promisify$2(fs$l.realpath);
const statMethods$1 = { lstat: lstat$1, stat: stat$7 };
...
function createFsWatchInstance(path2, options, listener, errHandler, emitRaw) {
  ...
  try {
    return fs$l.watch(path2, options, handleEvent);
  } catch (error2) {
    errHandler(error2);
  }
}
const fsWatchBroadcast = (fullPath, type2, val1, val2, val3) => {
...
const setFsWatchFileListener = (path2, fullPath, options, handlers) => {
  ...
  if (cont) {
    ...
    cont = {
      ...
      rawEmitters: rawEmitter,
      options,
      watcher: fs$l.watchFile(fullPath, options, (curr, prev) => {
  ...
  _handleFile(file2, stats, initialAdd) {
    ...
    const listener = async (path2, newStats) => {
      ...
      if (!newStats || newStats.mtimeMs === 0) {
        try {
          const newStats2 = await stat$7(file2);
    ...
    if (!(initialAdd && this.fsw.options.ignoreInitial) && this.fsw._isntIgnored(file2)) {
      if (!this.fsw._throttle(EV_ADD$2, file2, 0)) return;
...
const stat$6 = promisify$1(fs$k.stat);
const lstat = promisify$1(fs$k.lstat);
const realpath = promisify$1(fs$k.realpath);
const statMethods = { stat: stat$6, lstat };
...
const calcDepth = (path2, root2) => {
  let i = 0;
  while (!path2.indexOf(root2) && (path2 = sysPath$1.dirname(path2)) !== root2) i++;
  return i;
};
...
let FsEventsHandler$1 = class FsEventsHandler {
  ...
  async checkExists(path2, fullPath, realPath, parent, watchedDir, item, info, opts) {
    try {
      const stats = await stat$6(path2);
      if (this.fsw.closed) return;
...
const stat$5 = promisify(fs$j.stat);
const readdir = promisify(fs$j.readdir);
...
class FSWatcher extends EventEmitter {
  ...
  unwatch(paths_) {
    ...
    paths.forEach((path2) => {
      if (!sysPath.isAbsolute(path2) && !this._closers.has(path2)) {
        if (cwd2) path2 = sysPath.join(cwd2, path2);
  ...
  */
  ...
  async _emit(event, path2, val1, val2, val3) {
    ...
    if (isWindows$1) path2 = sysPath.normalize(path2);
  ...
  _awaitWriteFinish(path2, threshold, event, awfEmit) {
    ...
    const awaitWriteFinish = (prevStat) => {
      fs$j.stat(fullPath, (err, curStat) => {
...
class DirectoryWatcher {
  constructor(trigger, debounceMs = 2e3) {
  ...
  start(targetPath) {
    ...
    this.watcher = chokidar.watch(targetPath, {
      depth: 0,
      ignoreInitial: true,
      awaitWriteFinish: { stabilityThreshold: 1e3, pollInterval: 100 }
    });
...
const DATA_DIR = require$$1.app.getPath("userData");
const CONFIG_PATH = path$p.join(DATA_DIR, "config.json");
const RULES_PATH = path$p.join(DATA_DIR, "rules.json");
const STATS_PATH = path$p.join(DATA_DIR, "stats.json");
const LOG_PATH = path$p.join(DATA_DIR, "log.txt");
function readJson(filePath, fallback) {
  try {
    return JSON.parse(fs$n.readFileSync(filePath, "utf8"));
  } catch {
    return fallback;
  }
}
function writeJson(filePath, data) {
  fs$n.mkdirSync(path$p.dirname(filePath), { recursive: true });
  fs$n.writeFileSync(filePath, JSON.stringify(data, null, 2), "utf8");
}
function getConfig() {
  const saved = readJson(CONFIG_PATH, {});
  const config = { ...DEFAULT_CONFIG, ...saved };
  if (!config.targetPath) {
    config.targetPath = require$$1.app.getPath("downloads");
  }
  return config;
}
function setConfig(patch2) {
  const current = getConfig();
  writeJson(CONFIG_PATH, { ...current, ...patch2 });
}
function getRules() {
  return readJson(RULES_PATH, PRESETS.Default);
}
function setRules(rules) {
  writeJson(RULES_PATH, rules);
}
function getStats$3() {
  return readJson(STATS_PATH, { today: 0, allTime: 0, byCategory: {} });
}
function appendLog(line) {
  fs$n.mkdirSync(DATA_DIR, { recursive: true });
  fs$n.appendFileSync(LOG_PATH, line + "\n", "utf8");
}
function getLogs(maxLines = 0) {
  try {
    const content = fs$n.readFileSync(LOG_PATH, "utf8");
...
function clearLogs() {
  fs$n.writeFileSync(LOG_PATH, "", "utf8");
}
let activityLog = [];
function getActivity(limit = 10) {
  return activityLog.slice(0, limit);
}
function registerIpcHandlers(getWindow, organizer, watcher) {
  const emit = (channel, ...args) => {
    var _a;
    (_a = getWindow()) == null ? void 0 : _a.webContents.send(channel, ...args);
  };
  function log(line) {
    appendLog(line);
    emit("log", line);
  ...
  require$$1.ipcMain.handle("organize", async () => {
    const result = organizer.organize();
    emit("organize-complete", result);
    return result;
  });
  require$$1.ipcMain.handle("get-config", () => getConfig());
  require$$1.ipcMain.handle("set-config", (_e, patch2) => {
    setConfig(patch2);
    if ("debounceSeconds" in patch2 && patch2.debounceSeconds !== void 0) {
      watcher.setDebounce(patch2.debounceSeconds * 1e3);
    }
    if ("launchAtStartup" in patch2) {
      require$$1.app.setLoginItemSettings({ openAtLogin: patch2.launchAtStartup ?? false });
    }
  });
  require$$1.ipcMain.handle("get-rules", () => getRules());
  require$$1.ipcMain.handle("set-rules", (_e, rules) => {
    setRules(rules);
    organizer.updateRules(rules);
  });
  require$$1.ipcMain.handle("get-stats", () => ({
    ...getStats$3(),
    activity: getActivity(10)
  }));
  require$$1.ipcMain.handle("get-logs", (_e, maxLines) => getLogs(maxLines));
  require$$1.ipcMain.handle("clear-logs", () => clearLogs());
  require$$1.ipcMain.handle("start-watcher", () => {
  ...
  require$$1.ipcMain.handle("set-folder", (_e, folderPath) => {
  ...
  require$$1.ipcMain.handle("open-external", (_e, url) => {
    return require$$1.shell.openExternal(url);
  });
}
class AppTray {
  constructor(getWindow, onOrganize, onCheckUpdates) {
    __publicField(this, "tray", null);
    this.getWindow = getWindow;
    this.onOrganize = onOrganize;
    this.onCheckUpdates = onCheckUpdates;
  }
  create(watching, folderName) {
    if (this.tray) return;
    const iconPath = path$p.join(
      require$$1.app.isPackaged ? process.resourcesPath : "docs",
      "FileOrganizer.ico"
    );
    const icon = require$$1.nativeImage.createFromPath(iconPath);
    this.tray = new require$$1.Tray(icon.isEmpty() ? require$$1.nativeImage.createEmpty() : icon);
    this.tray.setToolTip(
      watching ? `FileOrganizer — Watching: ${folderName}` : "FileOrganizer — Idle"
    );
    this.updateMenu();
    this.tray.on("double-click", () => this.showWindow());
  }
  update(watching, folderName) {
    if (!this.tray) return;
    this.tray.setToolTip(
      watching ? `FileOrganizer — Watching: ${folderName}` : "FileOrganizer — Idle"
    );
  ...
  updateMenu() {
    var _a;
    const menu = require$$1.Menu.buildFromTemplate([
      ...
      { type: "separator" },
      { label: "Quit", click: () => require$$1.app.quit() }
    ]);
...
function setupUpdater(getWindow) {
  ...
  main$1.autoUpdater.on("update-available", (info) => {
    var _a;
    (_a = getWindow()) == null ? void 0 : _a.webContents.send("update-available", info.version);
  });
  ...
  setTimeout(() => {
    main$1.autoUpdater.checkForUpdates().catch(() => {
    });
  }, 3e3);
}
let mainWindow = null;
function createWindow() {
  mainWindow = new require$$1.BrowserWindow({
    ...
    webPreferences: {
      preload: path$p.join(__dirname, "preload.js"),
...
require$$1.app.whenReady().then(() => {
  ...
  const watcher = new DirectoryWatcher(
    () => organizer.organize(),
    config.debounceSeconds * 1e3
  );
  createWindow();
  const tray = new AppTray(
    () => mainWindow,
    () => organizer.organize(),
    () => require$$1.ipcMain.emit("check-for-updates")
  );
  tray.create(config.runInBackground, path$p.basename(config.targetPath));
  ...
  mainWindow.on("close", (e) => {
    ...
    if (cfg.runInBackground) {
      e.preventDefault();
      mainWindow.hide();
      tray.create(true, path$p.basename(cfg.targetPath));
    } else {
      tray.destroy();
    }
  });
